...
let getObject = function (req, key) {
  let value;
...
let saveObject = function (req, key, object) {
  if (req !== undefined && req.session !== undefined) {
...
let clearObject = function (req, key) {
  if (req !== undefined && req.session !== undefined) {
    req.session[key] = undefined;
  }
};

let saveUser = function (req, user) {
  saveObject(req, constant.SESSION_OBJECT_KEY.USER, user);
};

let getUser = function (req) {
  let user = getObject(req, constant.SESSION_OBJECT_KEY.USER);
  return user;
};

let clearUser = function (req) {
  clearObject(req, constant.SESSION_OBJECT_KEY.USER);
};
...
let requireLogin = function (req, res, next) {
  if (isLoggedIn(req)) {
    ...
    if (req.originalUrl.indexOf(constant.URL.REGISTER) > -1) {
      next();
    } else if (req.originalUrl.indexOf(constant.URL.LOGIN) > -1) {
      next();
    } else {
      res.status(constant.HTTP_STATUS_CODE.UNAUTHORIZED);
